  ...
  if (orderSuccess) return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <motion.div
        ...
        className="glass-card glow-yellow p-10 text-center max-w-sm w-full"
      >
        <motion.div
        ...
        <h2 className="text-xl font-bold mb-2 text-white">Order Placed!</h2>
        <p className="text-white/40 text-sm mb-6">Your order has been confirmed. Payment: Cash on Delivery.</p>
        <motion.button onClick={() => navigate('/market')} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="w-full py-3 bg-primary text-black font-bold text-sm rounded-full cursor-pointer border-none transition"
  ...
  if (items.length === 0) return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="glass-card p-10 text-center max-w-sm w-full">
        ...
        <h2 className="text-xl font-bold mb-2 text-white">Cart is Empty</h2>
        <p className="text-white/40 text-sm mb-6">Add products from the marketplace.</p>
        <motion.button onClick={() => navigate('/market')} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
          className="w-full py-3 bg-primary text-black font-bold text-sm rounded-full cursor-pointer border-none transition"
          style={{ boxShadow: '0 0 20px rgba(248,194,0,0.3)' }}>
          Browse Products
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="pt-20 pb-8 px-4 md:px-8 max-w-5xl mx-auto">
        <button onClick={() => navigate('/market')} className="text-white/40 text-sm mb-4 bg-transparent border-none cursor-pointer hover:text-white transition flex items-center gap-1">← Back to Market</button>
        <h2 className="text-xl font-bold mb-6 text-white">Checkout</h2>
        ...
        <div className="grid md:grid-cols-5 gap-6">
          ...
          <div className="md:col-span-3 flex flex-col gap-5">
            ...
            <div className="glass-card p-5">
              ...
              <div className="flex bg-white/5 rounded-full p-1 mb-4">
                <button onClick={() => setAddressMode('saved')} className={`flex-1 py-2 rounded-full text-sm font-medium border-none cursor-pointer transition ${addressMode === 'saved' ? 'bg-primary text-black' : 'bg-transparent text-white/40'}`}>Saved Addresses</button>
                <button onClick={() => setAddressMode('new')} className={`flex-1 py-2 rounded-full text-sm font-medium border-none cursor-pointer transition ${addressMode === 'new' ? 'bg-primary text-black' : 'bg-transparent text-white/40'}`}>One-time Address</button>
              </div>

              {addressMode === 'saved' ? (
                savedAddresses.length === 0 ? (
                  <div className="text-center py-6 text-white/40 text-sm">No saved addresses. Add one in your profile or enter a one-time address.</div>
                ) : (
                    ...
                    {savedAddresses.map(addr => (
                      <button key={addr.id} onClick={() => setSelectedAddrId(addr.id)}
                        ...
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-sm font-semibold capitalize text-white">{addr.label === 'other' ? (addr.custom_label || 'Other') : addr.label}</span>
                          {addr.is_default && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-primary/15 text-primary font-medium">Default</span>}
                        </div>
                        <p className="text-xs text-white/40 m-0 leading-relaxed">{addr.address_text}</p>
                      </button>
                    ))}
                  </div>
                )
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={handleUseMyLocation} disabled={locLoading}
                  className="glass-pill flex items-center gap-1.5 px-3 py-2 text-sm text-white/70 cursor-pointer hover:bg-white/10 transition disabled:opacity-50">
                  {locLoading ? <span className="w-3.5 h-3.5 border-2 border-white/20 border-t-primary rounded-full animate-spin" /> : '📍'} Use my location
                </button>
                <motion.button onClick={calculateDeliveryFees} disabled={feeLoading}
                  whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  className="flex-1 py-2 bg-primary text-black font-medium text-sm rounded-full cursor-pointer border-none transition disabled:opacity-50"
            ...
            {Object.entries(groupedByBusiness).map(([bizId, group]) => (
              <div key={bizId} className="glass-card p-5">
                <h4 className="text-sm font-semibold text-primary mb-3">{group.business_name}</h4>
                <div className="flex flex-col gap-2.5">
                  {group.items.map(({ product, quantity }) => (
                    <div key={product.id} className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-white/5 shrink-0 overflow-hidden">
                        {product.image1_url ? <img src={`${UPLOADS_URL}${product.image1_url.replace(/^\/uploads/, '')}`} className="w-full h-full object-cover" /> : null}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm m-0 truncate text-white">{product.name}</p>
                        <p className="text-xs text-white/40 m-0">₹{product.price}/{product.unit}</p>
                      </div>
                      <div className="flex items-center glass-pill overflow-hidden">
                        ...
                        <span className="w-4 text-center text-xs font-semibold text-white">{quantity}</span>
                        <button onClick={() => updateQuantity(product.id, quantity + 1)} className="w-6 h-6 border-none bg-transparent text-primary cursor-pointer text-sm font-bold">+</button>
                      </div>
                      <p className="text-sm font-medium m-0 w-16 text-right text-white">₹{(parseFloat(product.price) * quantity).toFixed(2)}</p>
                      <button onClick={() => removeFromCart(product.id)} className="text-red-400 text-xs bg-transparent border-none cursor-pointer">✕</button>
                    </div>
                  ))}
                </div>
                {feesCalculated && deliveryFees[bizId] && (
                  <div className="mt-3 pt-3 border-t border-white/8 text-sm">
                    {deliveryFees[bizId].outside
                      ? <span className="text-red-400">⚠ Outside range ({(deliveryFees[bizId].distance / 1000).toFixed(1)} km)</span>
                      : <span className="text-white/40">Delivery: ₹{deliveryFees[bizId].fee} ({(deliveryFees[bizId].distance / 1000).toFixed(1)} km)</span>}
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="md:col-span-2">
            <div className="glass-card p-5 md:sticky md:top-20" style={{ boxShadow: '0 -4px 20px rgba(248,194,0,0.1)' }}>
              <h3 className="text-base font-semibold mb-4 text-white">Bill Details</h3>
              <div className="flex justify-between text-sm text-white/40 mb-2"><span>Subtotal</span><span className="text-white">₹{subtotal.toFixed(2)}</span></div>
              <div className="flex justify-between text-sm text-white/40 mb-2"><span>Delivery Fee</span><span className="text-white">{feesCalculated ? (anyOutsideRange ? '—' : `₹${totalDeliveryFee.toFixed(2)}`) : '—'}</span></div>
              <div className="flex justify-between text-base font-bold mt-3 pt-3 border-t border-white/8"><span className="text-white">Grand Total</span><span className="text-primary">₹{grandTotal.toFixed(2)}</span></div>
              <div className="text-xs text-white/40 mt-3 flex items-center gap-1">💵 Cash on Delivery</div>
              {feesCalculated && (
                <motion.button onClick={handlePlaceOrder} disabled={orderLoading || anyOutsideRange}
                  whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                  className="w-full mt-5 py-3 bg-primary text-black font-bold text-sm rounded-full cursor-pointer border-none transition disabled:opacity-50"
                  style={{ boxShadow: '0 0 20px rgba(248,194,0,0.3)' }}>
